...
Data downloaded from PDC portal on August 9, 2021. All fields checked against PDC Data Dictionary.

Note for all PDC Data: "null" used as a value but is never a recorded option in data dictionary when there are also blank cells.


```{r, include = FALSE}
# Load the data
biospecimen = read.csv("../data/pdc/PDC_biospecimen_manifest_08092021_112122.csv")
studies = read.csv("../data/pdc/PDC_study_manifest_08092021_112119.csv")
clinical = read.csv("../data/pdc/PDC_clinical_manifest_08092021_112133.csv")
```
...
#### **analyte_type**
```{r, echo = FALSE}
bio <- data.frame(table(biospecimen$Analyte.Type)) 

bio %>%
  select(`Analyte Type` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 1.1: Analyte Type Value Distribution")
```

Note that there is an analyte_type field in both Aliquots and Analytes but only one field in the data table, "Analyte Type", which is not an exact match with the field name.

10 permissible values, all unused.

#### **analyte_type_id**

No exact or similar fields found within biospecimen manifest for this field.
...
#### **status**
```{r, echo = FALSE}
bio <- data.frame(table(biospecimen$Aliquot.Status)) 

bio %>%
  select(`Aliquot Status` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 1.2: Aliquot Status Value Distribution")
```

In the data table field name is "Aliquot Status" which is not an exact match to data dictionary entry "status".

2 permissible values, all used.

#### **aliquot_is_ref**
```{r, echo = FALSE}
bio <- data.frame(table(biospecimen$Aliquot.Is.Ref)) 

bio %>%
  select(`Aliquot is Ref` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 1.3: Aliquot Is Ref Value Distribution")
```

Field name is "Aliquot Is Ref" in data table, not exact match to data dictionary entry "aliquot_is_ref".

2 permissible values, all used.

### Analyte

Only 1 analyte type field found reported above. No analtye_type_id found.
...
##### **sample_type**
```{r, echo = FALSE}
bio <- data.frame(table(biospecimen$Sample.Type)) 

bio %>%
  select(`Sample Type` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 1.4: Sample Type Value Distribution")
enum <- c("Additional Metastatic", "Additional - New Primary", "Benign Neoplasms", "Blood Derived Cancer - Bone Marrow", "Blood Derived Cancer - Bone Marrow, Post-treatment", "Blood Derived Cancer - Peripheral Blood", "Blood Derived Cancer - Peripheral Blood, Post-treatment", "Blood Derived Liquid Biopsy", "Blood Derived Normal", "Bone Marrow Normal", "Buccal Cell Normal", "Cell Line Derived Xenograft Tissue", "Cell Lines", "Control Analyte", "DNA", "EBV Immortalized Normal", "Expanded Next Generation Cancer Model", "FFPE Recurrent", "FFPE Scrolls", "Fibroblasts from Bone Marrow Normal", "GenomePlex (Rubicon) Amplified DNA", "Granulocytes", "Human Tumor Original Cells", "In Situ Neoplasms", "Lymphoid Normal", "Metastatic", "Mononuclear Cells from Bone Marrow Normal", "Normal Adjacent Tissue", "Neoplasms of Uncertain and Unknown Behavior", "Next Generation Cancer Model", "Primary Blood Derived Cancer - Peripheral Blood", "Recurrent Blood Derived Cancer - Peripheral Blood", "Pleural Effusion", "Primary Blood Derived Cancer - Bone Marrow", "Primary Tumor", "Primary Xenograft Tissue", "Post neo-adjuvant therapy", "Recurrent Blood Derived Cancer - Bone Marrow", "Recurrent Tumor", "Repli-G (Qiagen) DNA", "Repli-G X (Qiagen) DNA", "RNA", "Slides", "Solid Tissue Normal", "Total RNA", "Tumor Adjacent Normal - Post Neo-adjuvant Therapy", "Tumor", "Xenograft Tissue", "Unknown", "Not Reported", "Not Allowed To Collect")
```

Field name is "Sample Type" in data table, not exact match to data dictionary entry "sample_type".

##### Unused Values:

`r length(enum)` permissible values, `r length(enum) - length(bio$Var1 %in% enum)` unused values listed below.

`r pander(enum[enum %notin% bio$Var1])`

#### **tissue_type**
```{r, echo = FALSE}
bio <- data.frame(table(biospecimen$Tissue.Type)) 

bio %>%
  select(`Tissue Type` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 1.5: Tissue Type Value Distribution")
```

Field name is "Tissue Type" in data table, not exact match to data dictionary entry "tissue_type".

7 permissible values, all unused.

#### **tissue_type**
```{r, echo = FALSE}
bio <- data.frame(table(biospecimen$Biospecimen.Anatomic.Site)) 

bio %>%
  select(`Biospecimen Anatomic Site` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 1.6: Biospecimen Anatomic Site Value Distribution")
```

Field name is "Biospecimen Anatomic Site" in data table, not exact match to data dictionary entry "biospecimen_anatomic_site".

285 permissible values, all unused.
...
#### **composition**
```{r, echo = FALSE}
bio <- data.frame(table(biospecimen$Composition)) 

bio %>%
  select(`Composition` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 1.7: Composition Value Distribution")
```

Field name is "Composition" in data table, not exact match to data dictionary entry "composition".

33 permissible values, all unused.

#### **diagnosis_pathologically_confirmed**
```{r, echo = FALSE}
bio <- data.frame(table(biospecimen$Diagnosis.Pathologically.Confirmed)) 

bio %>%
  select(`Diagnosis Pathoogically Confirmed` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 1.8: Diagnosis Pathologically Confirmed Value Distribution")
```

Field name is "Diagnosis Pathologically Confirmed" in data table, not exact match to data dictionary entry "diagnosis_pathologically_confirmed".

5 permissible values, all unused.
...
#### **is_ffpe**
```{r, echo = FALSE}
bio <- data.frame(table(biospecimen$Is.FFPE)) 

bio %>%
  select(`Is FFPE` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 1.9: Is FFPE Value Distribution")
```

Field name is "Is FFPE" in data table, not exact match to data dictionary entry "is_ffpe".

2 values total, both unused.

#### **method_of_sample_procurement**
```{r, echo = FALSE}
bio <- data.frame(table(biospecimen$Method.Of.Sample.Procurement)) 

bio %>%
  select(`Method Of Sample Procurement` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 1.10: Method of Sample Procurement Value Distribution")
```

Field name is "Method of Sample Procurement" in data table, not exact match to data dictionary entry "method_sample_procurement".

77 permissible values, all unused.

#### **preservation_method**
```{r, echo = FALSE}
bio <- data.frame(table(biospecimen$Preservation.Method)) 

bio %>%
  select(`Preservation Method` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 1.11: Preservation Method Value Distribution")
```

Field name is "Preservation Method" in data table, not exact match to data dictionary entry "preservation_method".

9 permissible values, all unused.

#### **sample_type_id**
```{r, echo = FALSE}
bio <- data.frame(table(biospecimen$Sample.Type.id)) 

bio %>%
  select(`Sample Type ID` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 1.12: Sample Type ID Value Distribution")
```

Field name is "Sample Type ID" in data table, not exact match to data dictionary entry "sample_type_id".

31 permissible values, all unused.

#### **tumor_code**
```{r, echo = FALSE}
bio <- data.frame(table(biospecimen$Tumor.Code)) 

bio %>%
  select(`Tumor Code` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 1.13: Tumor Code Value Distribution")
```

Field name is "Tumor Code" in data table, not exact match to data dictionary entry "tumor_code".

25 permissible values, all unused.


#### **tumor_code_id**
```{r, echo = FALSE}
bio <- data.frame(table(biospecimen$Tumor.Code.ID)) 

bio %>%
  select(`Tumor Code ID` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 1.14: Tumor Code ID Value Distribution")
```

Field name is "Tumor Code ID" in data table, not exact match to data dictionary entry "tumor_code_id".

25 permissible values, all unused.

#### **tumor_descriptor**
```{r, echo = FALSE}
bio <- data.frame(table(biospecimen$Tumor.Descriptor)) 

bio %>%
  select(`Tumor Descriptor` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 1.15: Tumor Descriptor Value Distribution")
```

Field name is "Tumor Descriptor" in data table, not exact match to data dictionary entry "tumor_descriptor".
...
#### **sample_is_ref**
```{r, echo = FALSE}
bio <- data.frame(table(biospecimen$Sample.Is.Ref)) 

bio %>%
  select(`Sample Is Ref` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 1.16: Sample Is Ref Value Distribution")
```

Field name is "Sample Is Ref" in data table, not exact match to data dictionary entry "sample_is_ref".

2 permissible values, both unused.
...
#### **ethnicity**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Ethnicity)) 

clin %>%
  select(`Ethnicity` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.1: Ethnicity Value Distribution")
```

Field name is "Ethnicity" in data table, not exact match to data dictionary entry "ethnicity".

Note that the values used in the data table are capitalized when this is not the case in the data dictionary.

##### Unused Values:

6 permissible values, 3 unused.
...
#### **gender**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Gender)) 

clin %>%
  select(`Gender` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.2: Gender Value Distribution")
```

Field name is "Gender" in data table, not exact match to data dictionary entry "gender".

Same note about capitalization above applies here.

##### Unused Values:

5 permissible values, 1 unused.
* unknown

#### **race**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Race)) 

clin %>%
  select(`Race` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.3: Race Value Distribution")
```

Field name is "Race" in data table, not exact match to data dictionary entry "race".

Same note about capitalization above applies here.

##### Unused Values:

10 values listed in data dictionary, 3 unused listed below: "Chinese" added as field value, but not reported in data dictionary.
* unknown
* not allowed to collect
* other
...
#### **cause_of_death**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Cause.of.Death)) 

clin %>%
  select(`Cause of Death` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.4: Cause of Death Value Distribution")
```

Field name is "Cause of Death" in data table, not exact match to data dictionary entry "cause_of_death".

##### Unused Values:

11 values listed in data dictionary, 4 unused listed below:
...
Values in data table but not listed in data dictionary:
...
No exact or similar fields found within clinical manifest for this field.

#### **vital_status**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Vital.Status)) 

clin %>%
  select(`Vital status` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.5: Vital Status Value Distribution")
```

Field name is "Vital Status" in data table, not exact match to data dictionary entry "vital_status".

##### Unused Values:

4 permissible values, 1 unused
* Unknown

Values in data table not listed in data dictionary:
*alive

#### **cause_of_death_source**

No exact or similar fields found within clinical manifest for cause_of_death_source field.

### Diagnosis

#### **last_known_disease_status**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Last.Known.Disease.Status)) 

clin %>%
  select(`Last Known Disease Status` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.6: Last Known Disease Status Value Distribution")
```

Field name is "Last Known Disease Status" in data table, not exact match to data dictionary entry "last_known_disease_status".

##### Unused Values

8 permissible values, 5 unused
* Distance met recurrence/progression
* Loco-regional recurrence/progression
* Biochemical evidence of disease without structural correlate
* not reported
* Not Allowed To Collect

Values in data table not listed in data dictionary or not exact matches:
* Tumor Free
...
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Progession.or.Recurrence)) 

clin %>%
  select(`Progression or Recurrence` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.7: Progression or Recurrence Value Distribution")
```

Field name is "Progression or Recurrence" in data table, not exact match to data dictionary entry "progression_or_recurrence".

##### Unused Values:

4 permissible values, 3 unused:
...
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Site.of.Resection.or.Biopsy)) 

clin %>%
  select(`Site of Resection or Biopsy` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.8: Site of Resection or Biopsy Value Distribution")
...
Field name is "Site of Resection or Biopsy" in data table, not exact match to data dictionary entry "site_of_resection_or_biopsy".
...
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Tissue.or.Organ.of.Origin)) 

clin %>%
  select(`Tissue or Organ of Origin` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.9: Tissue or Organ of Origin Value Distribution")

enum <- c("Abdomen, NOS", "Abdominal esophagus", "Accessory sinus, NOS", "Acoustic nerve", "Adrenal gland, NOS", "Ampulla of Vater", "Anal canal", "Anterior 2/3 of tongue, NOS", "Anterior floor of mouth", "Anterior mediastinum", "Anterior surface of epiglottis", "Anterior wall of bladder", "Anterior wall of nasopharynx", "Anus, NOS", "Aortic body and other paraganglia", "Appendix", "Ascending colon", "Autonomic nervous system, NOS", "Axillary tail of breast", "Base of tongue, NOS", "Bladder neck", "Bladder, NOS", "Blood", "Body of pancreas", "Body of penis", "Body of stomach", "Bone marrow", "Bone of limb, NOS", "Bone, NOS", "Bones of skull and face and associated joints", "Border of tongue", "Brain stem", "Brain, NOS", "Branchial cleft", "Breast, NOS", "Broad ligament", "Cardia, NOS", "Carotid body", "Cauda equina", "Cecum", "Central portion of breast", "Cerebellum, NOS", "Cerebral meninges", "Cerebrum", "Cervical esophagus", "Cervix uteri", "Choroid", "Ciliary body", "Cloacogenic zone", "Colon, NOS", "Commissure of lip", "Conjunctiva", "Connective, subcutaneous and other soft tissues of abdomen", "Connective, subcutaneous and other soft tissues of head, face, and neck", "Connective, subcutaneous and other soft tissues of lower limb and hip", "Connective, subcutaneous and other soft tissues of pelvis", "Connective, subcutaneous and other soft tissues of thorax", "Connective, subcutaneous and other soft tissues of trunk, NOS", "Connective, subcutaneous and other soft tissues of upper limb and shoulder", "Connective, subcutaneous and other soft tissues, NOS", "Cornea, NOS", "Corpus uteri", "Cortex of adrenal gland", "Cranial nerve, NOS", "Craniopharyngeal duct", "Descended testis", "Descending colon", "Dome of bladder", "Dorsal surface of tongue, NOS", "Duodenum", "Endocervix", "Endocrine gland, NOS", "Endometrium", "Epididymis", "Esophagus, NOS", "Ethmoid sinus", "Exocervix", "External ear", "External lip, NOS", "External lower lip", "External upper lip", "Extrahepatic bile duct", "Eye, NOS", "Eyelid", "Fallopian tube", "Female genital tract, NOS", "Floor of mouth, NOS", "Frontal lobe", "Frontal sinus", "Fundus of stomach", "Fundus uteri", "Gallbladder", "Gastric antrum", "Gastrointestinal tract, NOS", "Glans penis", "Glottis", "Greater curvature of stomach, NOS", "Gum, NOS", "Hard palate", "Head of pancreas", "Head, face or neck, NOS", "Heart", "Hematopoietic system, NOS", "Hepatic flexure of colon", "Hypopharyngeal aspect of aryepiglottic fold", "Hypopharynx, NOS", "Ileum", "Ill-defined sites within respiratory system", "Intestinal tract, NOS", "Intra-abdominal lymph nodes", "Intrahepatic bile duct", "Intrathoracic lymph nodes", "Islets of Langerhans", "Isthmus uteri", "Jejunum", "Kidney, NOS", "Labium majus", "Labium minus", "Lacrimal gland", "Laryngeal cartilage", "Larynx, NOS", "Lateral floor of mouth", "Lateral wall of bladder", "Lateral wall of nasopharynx", "Lateral wall of oropharynx", "Lesser curvature of stomach, NOS", "Lingual tonsil", "Lip, NOS", "Liver", "Long bones of lower limb and associated joints", "Long bones of upper limb, scapula and associated joints", "Lower gum", "Lower limb, NOS", "Lower lobe, lung", "Lower third of esophagus", "Lower-inner quadrant of breast", "Lower-outer quadrant of breast", "Lung, NOS", "Lymph node, NOS", "Lymph nodes of axilla or arm", "Lymph nodes of head, face and neck", "Lymph nodes of inguinal region or leg", "Lymph nodes of multiple regions", "Main bronchus", "Major salivary gland, NOS", "Male genital organs, NOS", "Mandible", "Maxillary sinus", "Meckel diverticulum", "Mediastinum, NOS", "Medulla of adrenal gland", "Meninges, NOS", "Middle ear", "Middle lobe, lung", "Middle third of esophagus", "Mouth, NOS", "Mucosa of lip, NOS", "Mucosa of lower lip", "Mucosa of upper lip", "Myometrium", "Nasal cavity", "Nasopharynx, NOS", "Nervous system, NOS", "Nipple", "Occipital lobe", "Olfactory nerve", "Optic nerve", "Orbit, NOS", "Oropharynx, NOS", "Other ill-defined sites", "Other specified parts of female genital organs", "Other specified parts of male genital organs", "Other specified parts of pancreas", "Ovary", "Overlapping lesion of accessory sinuses", "Overlapping lesion of bladder", "Overlapping lesion of bones, joints and articular cartilage", "Overlapping lesion of bones, joints and articular cartilage of limbs", "Overlapping lesion of brain", "Overlapping lesion of brain and central nervous system", "Overlapping lesion of breast", "Overlapping lesion of cervix uteri", "Overlapping lesion of colon", "Overlapping lesion of connective, subcutaneous and other soft tissues", "Overlapping lesion of corpus uteri", "Overlapping lesion of digestive system", "Overlapping lesion of endocrine glands and related structures", "Overlapping lesion of esophagus", "Overlapping lesion of eye and adnexa", "Overlapping lesion of female genital organs", "Overlapping lesion of floor of mouth", "Overlapping lesion of heart, mediastinum and pleura", "Overlapping lesion of hypopharynx", "Overlapping lesion of ill-defined sites", "Overlapping lesion of larynx", "Overlapping lesion of lip", "Overlapping lesion of lip, oral cavity and pharynx", "Overlapping lesion of lung", "Overlapping lesion of major salivary glands", "Overlapping lesion of male genital organs", "Overlapping lesion of nasopharynx", "Overlapping lesion of other and unspecified parts of mouth", "Overlapping lesion of palate", "Overlapping lesion of pancreas", "Overlapping lesion of penis", "Overlapping lesion of peripheral nerves and autonomic nervous system", "Overlapping lesion of rectum, anus and anal canal", "Overlapping lesion of respiratory system and intrathoracic organs", "Overlapping lesion of retroperitoneum and peritoneum", "Overlapping lesion of skin", "Overlapping lesion of small intestine", "Overlapping lesion of stomach", "Overlapping lesion of tongue", "Overlapping lesion of tonsil", "Overlapping lesion of urinary organs", "Overlapping lesion of vulva", "Overlapping lesions of oropharynx", "Palate, NOS", "Pancreas, NOS", "Pancreatic duct", "Parametrium", "Parathyroid gland", "Paraurethral gland", "Parietal lobe", "Parotid gland", "Pelvic bones, sacrum, coccyx and associated joints", "Pelvic lymph nodes", "Pelvis, NOS", "Penis, NOS", "Peripheral nerves and autonomic nervous system of abdomen", "Peripheral nerves and autonomic nervous system of head, face, and neck", "Peripheral nerves and autonomic nervous system of lower limb and hip", "Peripheral nerves and autonomic nervous system of pelvis", "Peripheral nerves and autonomic nervous system of thorax", "Peripheral nerves and autonomic nervous system of trunk, NOS", "Peripheral nerves and autonomic nervous system of upper limb and shoulder", "Peritoneum, NOS", "Pharynx, NOS", "Pineal gland", "Pituitary gland", "Placenta", "Pleura, NOS", "Postcricoid region", "Posterior mediastinum", "Posterior wall of bladder", "Posterior wall of hypopharynx", "Posterior wall of nasopharynx", "Posterior wall of oropharynx", "Prepuce", "Prostate gland", "Pylorus", "Pyriform sinus", "Rectosigmoid junction", "Rectum, NOS", "Renal pelvis", "Reticuloendothelial system, NOS", "Retina", "Retromolar area", "Retroperitoneum", "Rib, sternum, clavicle and associated joints", "Round ligament", "Scrotum, NOS", "Short bones of lower limb and associated joints", "Short bones of upper limb and associated joints", "Sigmoid colon", "Skin of lip, NOS", "Skin of lower limb and hip", "Skin of other and unspecified parts of face", "Skin of scalp and neck", "Skin of trunk", "Skin of upper limb and shoulder", "Skin, NOS", "Small intestine, NOS", "Soft palate, NOS", "Specified parts of peritoneum", "Spermatic cord", "Sphenoid sinus", "Spinal cord", "Spinal meninges", "Spleen", "Splenic flexure of colon", "Stomach, NOS", "Subglottis", "Sublingual gland", "Submandibular gland", "Superior wall of nasopharynx", "Supraglottis", "Tail of pancreas", "Temporal lobe", "Testis, NOS", "Thoracic esophagus", "Thorax, NOS", "Thymus", "Thyroid gland", "Tongue, NOS", "Tonsil, NOS", "Tonsillar fossa", "Tonsillar pillar", "Trachea", "Transverse colon", "Trigone of bladder", "Undescended testis", "Unknown primary site", "Upper Gum", "Upper limb, NOS", "Upper lobe, lung", "Upper respiratory tract, NOS", "Upper third of esophagus", "Upper-inner quadrant of breast", "Upper-outer quadrant of breast", "Urachus", "Ureter", "Ureteric orifice", "Urethra", "Urinary system, NOS", "Uterine adnexa", "Uterus, NOS", "Uvula", "Vagina, NOS", "Vallecula", "Ventral surface of tongue, NOS", "Ventricle, NOS", "Vertebral column", "Vestibule of mouth", "Vulva, NOS", "Waldeyer ring", "Biliary tract, NOS", "Cheek mucosa", "Clitoris", "Overlapping lesion of biliary tract", "Unknown", "Not Reported")
```

Field name is "Tissue or Organ of Origin" in data table, not exact match to data dictionary entry "tissue_or_organ_of_origin".
...
#### **tumor_grade**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Tumor.Grade)) 

clin %>%
  select(`Ethnicity` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.10: Ethnicity Value Distribution")
```

Field name is "Tumor Grade" in data table, not exact match to data dictionary entry "tumor_grade".

##### Unused Values:
11 permissible values, 2 unused:
* GB
* not reported

#### **ajcc_clinical_m**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$AJCC.Clinical.M)) 

clin %>%
  select(`AJCC Clinical M` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.11: AJCC Clinical M Value Distribution")
```

Field name is "AJCC Clinical M" in data table, not exact match to data dictionary entry "ajcc_clinical_m".

##### Unused Values:

10 permissible values, 5 unused listed below:
* M1a
* M1b
* M1c
* cM0 (i+)
...
#### **ajcc_clinical_n**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$AJCC.Clinical.N)) 

clin %>%
  select(`AJCC Clinical N` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.12: AJCC Clinical N Value Distribution")

enum <- c("N0", "N0 (i+)", "N0 (i-)", "N0 (mol+)", "N0 (mol-)", "N1", "N1a", "N1b", "N1bI", "N1bII", "N1bIII", "N1bIV", "N1c", "N1mi", "N2", "N2a", "N2b", "N2c", "N3", "N3a", "N3b", "N3c", "N4", "NX", "Unknown", "Not Reported", "Not Allowed To Collect")
```

Field name is "AJCC Clinical N" in data table, not exact match to data dictionary entry "ajcc_clinical_n".
...
#### **ajcc_clinical_stage**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$AJCC.Clinical.Stage)) 

clin %>%
  select(`AJCC Clinical Stage` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.13: AJCC Clinical Stage Value Distribution")

enum <- c("Stage 0", "Stage 0a", "Stage 0is", "Stage I", "Stage IA", "Stage IA1", "Stage IA2", "Stage IB", "Stage IB Cervix", "Stage IB1", "Stage IB2", "Stage IC", "Stage II", "Stage II Cervix", "Stage IIA", "Stage IIA1", "Stage IIA2", "Stage IIA Cervix", "Stage IIB", "Stage IIC", "Stage IIC1", "Stage III", "Stage IIIA", "Stage IIIB", "Stage IIIC", "Stage IIIC1", "Stage IIIC2", "Stage IS", "Stage IV", "Stage IVA", "Stage IVB", "Stage IVC", "Stage Tis", "Stage X", "Unknown", "Not Reported", "Not Allowed To Collect")
```

Field name is "AJCC Clinical Stage" in data table, not exact match to data dictionary entry "ajcc_clinical_stage".
...
#### **ajcc_clinical_t**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$AJCC.Clinical.T)) 

clin %>%
  select(`AJCC Clinical T` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.14: AJCC Clinical T Value Distribution")

enum <- c("T0", "T1", "T1a", "T1a1", "T1a2", "T1b", "T1b1", "T1b2", "T1c", "T1mi", "T2", "T2a", "T2a1", "T2a2", "T2b", "T2c", "T2d", "T3", "T3a", "T3b", "T3c", "T3d", "T4", "T4a", "T4b", "T4c", "T4d", "T4e", "TX", "Ta", "Tis", "Tis (DCIS)", "Tis (LCIS)", "Tis (Paget's)", "Unknown", "Not Reported", "Not Allowed To Collect")
```

Field name is "AJCC Clinical T" in data table, not exact match to data dictionary entry "ajcc_clinical_t".
...
#### **ajcc_pathologic_m**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$AJCC.Pathologic.M)) 

clin %>%
  select(`AJCC Pathologic M` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.15: AJCC Pathologic M Value Distribution")

```

Field name is "AJCC Pathologic M" in data table, not exact match to data dictionary entry "ajcc_pathologic_m".

##### Unused Values:

11 permissible values, 7 unused values:
* M1a
* M1b
* M1c
* M2
...
```{r, echo = FALSE}
clin <- data.frame(table(clinical$AJCC.Pathologic.Stage)) 

clin %>%
  select(`AJCC Pathologic Stage` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.16: AJCC Pathologic Stage Value Distribution")

enum <- c("Stage 0", "Stage 0a", "Stage 0is", "Stage I", "Stage IA", "Stage IA1", "Stage IA2", "Stage IB", "Stage IB Cervix", "Stage IB1", "Stage IB2", "Stage IC", "Stage II", "Stage II Cervix", "Stage IIA", "Stage IIA1", "Stage IIA2", "Stage IIA Cervix", "Stage IIB", "Stage IIC", "Stage IIC1", "Stage III", "Stage IIIA", "Stage IIIB", "Stage IIIC", "Stage IIIC1", "Stage IIIC2", "Stage IS", "Stage IV", "Stage IVA", "Stage IVB", "Stage IVC", "Stage Tis", "Stage X", "Unknown", "Not Reported", "Not Allowed To Collect")
```

Field name is "AJCC Pathologic Stage" in data table, not exact match to data dictionary entry "ajcc_pathologic_stage".
...
```{r, echo = FALSE}
clin <- data.frame(table(clinical$AJCC.Staging.System.Edition)) 

clin %>%
  select(`AJCC Staging System Edition` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.17: AJCC Staging System Edition Value Distribution")

```

Field name is "AJCC Staging System Edition" in data table, not exact match to data dictionary entry "ajcc_staging_system_edition.

10 permissible values, all unused.

#### **anaplasia_present**

No exact or similar fields found within clinical manifest for anaplasia_present field.

#### **anaplasia_present_type**

No exact or similar fields found within clinical manifest for anaplasi_present_type field.

#### **ann_arbor_b_symptoms**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Ann.Arbor.B.Symptoms)) 

clin %>%
  select(`Ann Arbor B Symptoms` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.18: Ann Arbor B Symptoms Value Distribution")

```

Field name is "Ann Arbor B Symptoms" in data table, not exact match to data dictionary entry "ann_arbor_b_symptoms".

##### Unused Values:

5 permissible values, 4 unused:
* Yes
* No
* Unknown
* Not Allowed to Collect
...
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Ann.Arbor.Clinical.Stage)) 

clin %>%
  select(`Ann Arbor Clinical Stage` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.19: Ann Arbor Clinical Stage Value Distribution")

```

Field name is "Ann Arbor Clinical Stage" in data table, not exact match to data dictionary entry "ann_arbor_clinical_stage".

##### Unused Values:

6 permissible values, 5 unused:
* Stage I
* Stage II
* Stage III
* Stage IV
...
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Ann.Arbor.Extranodal.Involvement)) 

clin %>%
  select(`Ann Arbor Extranodal Involvement` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.20: Ann Arbor Extranodal Involvement Value Distribution")

```

Field name is "Ann Arbor Extranodal Involvement" in data table, not exact match to data dictionary entry "ann_arbor_extranodal_involvement".
...
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Ann.Arbor.Pathologic.Stage)) 

clin %>%
  select(`Ann Arbor Pathologic Stage` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.21: Ann Arbor Pathologic Stage Value Distribution")

```

Field name is "Ann Arbor Pathologic Stage" in data table, not exact match to data dictionary entry "ann_arbor_pathologic_stage".
...
```{r, echo = FALSE}
data.frame(table(clinical$Best.Overall.Response)) 

clin %>%
  select(`Best Overall Response` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.22: Best Overall Response Value Distribution")

```

Field name is "Best Overall Response" in data table, not exact match to data dictionary entry "best_overall_response".

26 permissible values, all unused.

#### **burkitt_lymphoma_clinical_variant**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Burkitt.Lymphoma.Clinical.Variant)) 

clin %>%
  select(`Burkitt Lymphoma Clinical Variant` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.23: Burkitt Lymphoma Clinical Variant Value Distribution")

```

Field name is "Burkitt Lymphoma Clinical Variant" in data table, not exact match to data dictionary entry "burkitt_lymphoma_clinical_variant".

##### Unused Values:

8 permissible values, 7 unused:
* Endemic
...
No matching or similar terms found within clinical manifest for this field

#### **classification_of_tumor**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Classification.of.Tumor)) 

clin %>%
  select(`Classification of Tumor` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.24: Classification of Tumor Value Distribution")

```

Field name is "Classification of Tumor" in data table, not exact match to data dictionary entry "classification_of_tumor".

##### Unused Values:

7 permissible values, all unused.

All field values in the data table are not listed in data dictionary.

#### **cog_liver_stage**

No exact or similar fields found within clinical manifest for cog_liver_stage field.

#### **cog_neuroblastoma_risk_group**

No exact or similar fields found within clinical manifest for cog_neuroblastoma_risk_group field.

#### **cog_renal_stage**

No exact or similar fields found within clinical manifest for cog_renal_stage field.

#### **cog_rhabdomyosarcoma_risk_group**

No exact or similar fields found within clinical manifest for cog_rhabdomyosarcoma field.

#### **enneking_msts_grade**

No exact or similar fields found within clinical manifest for enneking_msts_grade field.

#### **enneking_msts_metastasis**

No exact or similar fields found within clinical manifest for enneking_msts_metastasis field.

#### **enneking_msts_stage**

No exact or similar fields found within clinical manifest for enneking_msts_stage field.

#### **enneking_msts_tumor_site**

No exact or similar fields found within clinical manifest for enneking_msts_tumor_site field.

#### **esophageal_columnar_dysplasia_degree**

No exact or similar fields found within clinical manifest for esophageal_columnar_dysplasia_degree field.

#### **esophageal_columnar_metaplasia_present**

No exact or similar fields found within clinical manifest for esophageal_columnar_metaplasia_present field.

#### **figo_stage**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Figo.Stage)) 

clin %>%
  select(`Figo Stage` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.25: Figo Stage Value Distribution")

enum <- c("Stage 0", "Stage I", "Stage IA", "Stage IA1", "Stage IA2", "Stage IB", "Stage IB1", "Stage IB2", "Stage IC", "Stage II", "Stage IIA", "Stage IIA1", "Stage IIA2", "Stage IIB", "Stage IIC", "Stage III", "Stage IIIA", "Stage IIIB", "Stage IIIC", "Stage IIIC1", "Stage IIIC2", "Stage IV", "Stage IVA", "Stage IVB", "Unknown", "Not Reported", "Not Allowed To Collect")
```

Field name is "Figo Stage" in data table, not exact match to data dictionary entry "figo_stage".
...
No exact matching or similar fields found within clinical manifest for first_symptom_prior_to_diagnosis field.

#### **gastric_esophageal_junction_involvement**

No exact matching or similar fields found within clinical manifest for gastric_esophageal_junction_involvement field.

#### **goblet_cells_columnar_mucosa_present**

No exact matching or similar fields found within clinical manifest for goblet_cells_columnar_mucosa_present field.

#### **inpc_grade**

No exact matching or similar fields found within clinical manifest for inpc_grade field.

#### **inpc_histologic_group**

No exact matching or similar fields found within clinical manifest for inpc_histologic_group field.

#### **inrg_stage**

No exact matching or similar fields found within clinical manifest for inrg_stage field.

#### **inss_stage**

No exact matching or similar fields found within clinical manifest for inss_stage field.

#### **irs_group**

No exact matching or similar fields found within clinical manifest for irs_group field.

#### **irs_stage**

No exact matching or similar fields found within clinical manifest for irs_stage field.

#### **ishak_fibrosis_score**

No exact matching or similar fields found within clinical manifest for ishak_fibrosis_score field.

#### **iss_stage**

No exact matching or similar fields found within clinical manifest for iss_stage field.
...
clin %>%
  select(`Laterality` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.26: Laterality Value Distribution")


```

Field name is "Laterality" in data table, not exact match to data dictionary entry "laterality".

##### Unused Values:

7 permissible values, 6 unused.
* Bilateral
* Left
* Midline
* Right
...
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Lymphatic.Invasion.Present)) 

clin %>%
  select(`Lympathic Invasion Present` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.27: Lymphatic Invasion Present Value Distribution")


```

Field name is "Lymphatic Invasion Present" in data table, not exact match to data dictionary entry "lymphatic_invasion_present".
...
No exact match or similar fields found within clinical manifest for medulloblastoma_molecular_classification field.

#### **metastasis_at_diagnosis**

No exact matching or similar fields found within clinical manifest for metastasis_at_diagnosis field.

#### **metastasis_at_diagnosis_site**

No exact matching or similar fields found within clinical manifest for metastasis_at_diagnosis_site field.

#### **method_of_diagnosis**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Method.of.Diagnosis)) 

clin %>%
  select(`Method of Diagnosis` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.28: Method of Diagnosis Value Distribution")

enum <- c("Autopsy", "Biopsy", "Blood Draw", "Bone Marrow Aspirate", "Core Biopsy", "Cytology", "Cystoscopy", "Debulking", "Diagnostic Imaging", "Dilation and Curettage Procedure", "Enucleation", "Excisional Biopsy", "Fine Needle Aspiration", "Imaging", "Incisional Biopsy", "Laparoscopy", "Laparotomy", "Other", "Pap Smear", "Physical Exam", "Pathologic Review", "Surgical Resection", "Thoracentesis", "Ultrasound Guided Biopsy", "Unknown", "Not Reported", "Not Allowed To Collect")

```

Field name is "Method of Diagnosis" in data table, not exact match to data dictionary entry "method_of_diagnosis".
...
No exactly matching or similar fields found within clinical manifest for micorpapillary_features field.

#### **mitosis_karyorrhexis_index**

No exact matching or similar fields found within clinical manifest for mitosis_karyorrhexis field.

#### **perineural_invasion_present**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Perineural.Invasion.Present)) 

clin %>%
  select(`Perineural Invasion Present` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.29: Perineural Invasion Present Value Distribution")
```

Field name is "Perineural Invasion Present" in data table, not exact match to data dictionary entry "perineural_invasion_present".
...
No exact matching or similar fields found within clinical manifest for peripancreatic_lymph_nodes_positive field.

#### **prior_malignancy**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Prior.Malignancy)) 

clin %>%
  select(`Prior Malignancy` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.30: Prior Malignancy Value Distribution")
```

Field name is "Prior Malignancy" in data table, not exact match to data dictionary entry "prior_malignancy".

##### Unused Values:

5 permissible values, all unused.

Values present in data table but not listed in data dictionary:
* No
* Not Reported
* Unknown
* Yes

Note main difference is capitalization

#### **prior_treatment**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Prior.Treatment)) 

clin %>%
  select(`Prior Treatment` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.31: Prior Treatment Value Distribution")
```

Field name is "Prior Treatment" in data table, not exact match to data dictionary entry "prior_treatment".

##### Unused Values:
5 permissible values, 2 unused:
* Unknown
* Not Allowed To Collect

#### **residual_disease**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Residual.Disease)) 

clin %>%
  select(`Residual Disease` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.32: Residual Disease Value Distribution")
```

Field name is "Residual Disease" in data table, not exact match to data dictionary entry "residual_disease".

##### Unused Values:
6 permissible values, 1 unused:
* Unknown

#### **supratentorial_localization**

No exact matching or similar fields found within clinical manifest for supratentorial_localization field.

#### **synchronous_malignancy**

No exact matching or similar fields found within clinical manifest for synchronous_malignancy field.

#### **tumor_confined_to_tumor_of_origin**

No exact matching or similar fields found within clinical manifest for this field.
...
```{r, echo = FALSE}
clin <- data.frame(table(clinical$Vascular.Invasion.Present)) 

clin %>%
  select(`Vascular Invasion Present` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.33: Vascular Invasion Present Value Distribution")
```

Field name is "Vascular Invasion Present" in data table, not exact match to data dictionary entry "vascular_invasion_present".
...
No enumerated fields in Family History were found within clinical manifest

### Follow-Up

Fields in data dictionary not found within clinical manifest:

* adverse_effect
* barrets_esophagus_goblet_cells_present
* comorbidity
...
#### **hpv_positive_type**
```{r, echo = FALSE}
clin <- data.frame(table(clinical$HPV.Positive.Type)) 

clin %>%
  select(`HPV Positive Type` = Var1, `Frequency` = Freq) %>%
  kable(format = "pipe",
        caption = "Table 2.34: HPV Positive Type Value Distribution")
```

23 permissible values, all unused.

Field name is "HPV Positive Type" in data table, not exact match to data dictionary entry "hpv_positive_type".

#### **progression_or_recurrence**

Only one such field found in clinical manifest, detailed under Diagnosis.

### Exposure

No enumerated fields from this category present in clinical manifest.
